...
export default {
  ...
  async mounted() {
    ...
    renderer.setAnimationLoop(() => {
      ...
      // Make camera follow selected planet
      if (selectedPlanet) {
  ...
  computed: {
    date() {
      // Create a date object based on the provided time
      const date = new Date(this.time);

      // Calculate the offset for Bangladesh Standard Time (BST), which is UTC+6
      const bangladeshOffset = 6 * 60; // 6 hours in minutes

      // Convert the time to UTC and then apply the BST offset
      const utcOffset = date.getTimezoneOffset();
      date.setMinutes(date.getMinutes() + utcOffset + bangladeshOffset);
      ...
      const result = `${day}. ${month}. ${year} - ${hours}:${minutes}:${seconds} (BST)`;
